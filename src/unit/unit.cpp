...
inline void bClassUnit::draw_sprite() //Draw the unit's sprite.
{
	//cout << "In unit draw...\n";
	...
	{
		//cout << "1...\n";
		if(!animation_playing) //If an animation is not currently playing.
		{
			//cout << "2...\n";
			draw_frame = fps_counter; //Let the game know the unit has already been drawn this frame.
			if(selected == false) //If the unit is not selected...
			{
				draw(wx - (PCamera->wx), wy - (PCamera->wy), sprite, screen); //Draw the 'normal' sprite.
			}
			else if(selected) //If it is selected...
			{
				draw(wx - (PCamera->wx), wy - (PCamera->wy), sprite_select, screen); //Draw the selected sprite...
			}
			if(player) //If this is the unit the player is controlling...
			{
				draw_inventory(); //Let the player know what tools this guy is carrying.
			}
		}
		else //So, an animation is playing...
		{
			//TODO: Find out what animation is playing.
		}
	}
	if(selected) //If the unit is selected...
	{
...
std::string bClassUnit::update()
{
	...
	//if(screen_needs_updating == false) //So, if the screen is not pending updation by the draw thread...TODO: Make this work.
	//{
		draw_sprite(); //Draw the unit's sprite.
	//}
